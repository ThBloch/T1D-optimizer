...
## 2026-06-22 - E8: Dexcom Developer API v3 implemented as authoritative insulin + overnight-glucose source
**Status:** accepted
**Decision:** Two new scripts ship: `dexcom_events_fetch.py` (OAuth fetcher, incremental by default, `--full` seeds) and `dexcom_events_loader.py` (offline cache reader). Auth via `~/.dexcom_api/` (config.json + tokens.json); refresh_token is rotated on each refresh and persisted with computed `expires_at`. Cache at `data/dexcom_api/events.json` + `egvs.json` (gitignored). EU server: `api.dexcom.eu`. API scope decisions made this session: (1) basal (`longActing`) + bolus (`fastActing`, regular-pen era Apr-May 2025) both pulled from API; (2) overnight EGV window (job 2) moves to Developer API - rolling 30-day cache, 6h chunks (~72 records/chunk, safely under API's ~100-record cap per request); live dose-time reading stays on pydexcom Share API (3h EU delay makes Developer API unusable for real-time). Anchor priority changed to: API (1) -> Clarity CSV (2) -> diary (3) -> --dose flag (4). `load_bolus_combined()` now prefers API fastActing from its earliest covered date; Clarity Hurtig only for dates before that cutover; Glooko always disjoint. `tests/test_dexcom_fetch.py` updated with new stubs (`load_api_glucose`, `load_api_basal`) + 1 new case (`test_api_basal_used_as_anchor_priority_1`); `tests/test_dexcom_events_loader.py` added (22 cases). Reconciliation: 0 mismatches on 390 overlapping basal dates (Clarity vs API); 100/100 EGV values match within 0.1 mmol/L on a shared overnight window (API mg/dL->mmol/L conversion confirmed correct). dose.md updated: step 0 refreshes API cache before `dexcom_fetch.py`. Technical limits discovered: events endpoint returns HTTP 400 for spans > ~36 days (using 30-day chunks); egvs truncates at ~100 records per request with no pagination token (using 6h chunks).
**Why:** Manual Clarity CSV export was the last remaining toil in the nightly dose flow. The Developer API is the same underlying data source (G7-app-logged insulin events), OAuth-sanctioned, and covers all dates from diagnosis. Moving the overnight EGV window to the API removes the Share API's 24h boundary fragility (risk of missing early-night readings if run late). Clarity CSVs remain as deep-history fallback for research scripts.

## 2026-06-05 - Production-path smoke test added (E14)
**Status:** accepted
**Decision:** `tests/test_dexcom_fetch.py` added with 3 fixture-based smoke tests covering the `dexcom_fetch.run()` end-to-end path without live API or real medical data. `run()` refactored to accept an optional args list (`def run(args=None)`) so tests can inject CLI flags via `parser.parse_args(args)` without patching sys.argv - standard Python argparse pattern, existing `run()` call with no args unchanged. Stubs via `unittest.mock.patch`: `fetch_readings` (returns synthetic CGM list), `load_creds` (dummy dict), `load_dexcom` (raises FileNotFoundError - Clarity branch skips gracefully), `load_bolus_combined` (empty list or synthetic bolus event), `load_whoop` (returns dict with today's strain), `dose_diary.DIARY_PATH` (temp file - redirects load_diary + save_diary). Three cases: (1) normal night produces suggestion; (2) missing strain emits NEEDS: strain; (3) falling slope + bolus in second half flags ambiguity in output.
**Why:** E12 (NEEDS-line protocol) and E19 (bolus disambiguator wiring) both modified the production path with zero integration coverage. The 49 unit tests cover `thomas_rules()` exhaustively but cannot catch import errors, signature mismatches, or incorrect wiring between `dexcom_fetch` and its dependencies. Smoke tests catch those classes of regression at `py -X utf8 tests/test_dexcom_fetch.py` time rather than at live `/dose` time.

## 2026-06-05 - Bolus disambiguator added to thomas_rules (E19, P8 enforcement)
**Status:** accepted
**Decision:** `thomas_rules()` now accepts an optional `bolus_in_second_half` parameter (list of `(datetime, units)` tuples, default `None`). When `sh_slope < 0` AND `bolus_in_second_half` is non-empty, the negative-slope dose reduction is skipped and a warning line is added to reasoning instead ("slope ambiguous, no reduction"). When `bolus_in_second_half` is empty or None, the existing -1u/-2u logic fires unchanged. Positive slope branches and fasting fallback branches are unaffected. The calling code in `dexcom_fetch.py` loads all bolus events via `load_bolus_combined()`, computes the second-half start using `SECOND_HALF_FRACTION` (imported from `night_stats`), and passes the pre-filtered list. `thomas_rules()` itself is kept pure (no I/O, just checks truthiness and len of the list it receives).

**Why:** P8 (code-conventions.md) requires that when a signal's interpretation depends on a disambiguator, the disambiguator is a required input. A falling overnight slope can mean "basal too high" OR "correction bolus taken mid-night" - without bolus context the rule cannot distinguish the two cases and may incorrectly reduce the next dose after a night where Thomas corrected a high. Bolus data is available via `load_bolus_combined()` (both Clarity manual and Glooko NovoPen streams); the missing piece was wiring it into the rule layer. Flag-in-reasoning was chosen over refuse-to-suggest because the data IS available (unlike the E12 strain case where data was genuinely absent); skipping the slope adjustment when a bolus is present is the safe default without denying a suggestion entirely.
...
## 2026-06-06 - NEEDS-line protocol completed; script fully non-interactive (E1b + E1d)
**Status:** accepted
**Decision:** `dexcom_fetch.py` is now fully non-interactive. The Priority-3 dose-prompt block (the only remaining `input()` call) is replaced with `print("NEEDS: dose")` + `save_diary()` + `return`. `.claude/commands/dose.md` steps 3-4 are updated to specify the exact prompt wording and validation rules for each `NEEDS:` signal:
- `NEEDS: dose`: ask "What dose did you take last night? (units)"; must be a positive number; re-ask if blank or non-numeric.
- `NEEDS: strain`: ask "WHOOP strain for today (check app):"; must be a number (float accepted, e.g. 12.4); re-ask if non-numeric; warn "Outside normal WHOOP range (0-21), proceeding anyway" if value is outside 0-21 but still accept it.
- Order when both are missing: dose first, then strain (dose is the anchor without which no suggestion is possible).
- Prompts live inline in `.claude/commands/dose.md` (not a separate doc); deferred to `docs/dose-manual-prompts.md` when E6 (Telegram) needs them.

`tests/test_dexcom_fetch.py` gains one new smoke test (`test_needs_dose_emitted_when_no_anchor`) covering the `NEEDS: dose` path. Total: 4 smoke tests, all green.
**Why:** E12 (2026-05-29) enforced the strain-non-negotiable invariant by making the script signal `NEEDS: strain` and refuse. The dose-prompt `input()` call was the remaining interactive block - a hard blocker for any non-interactive consumer (cron jobs, the future Telegram bot). Replacing it with a `NEEDS: dose` signal completes the NEEDS-line protocol: the script is now a pure stdin-free CLI that always exits cleanly and communicates missing inputs via stdout. The slash command (`/dose`) owns all user interaction; the script owns all data loading and computation. This separation is the correct layer split for the eventual E6 Telegram path (the bot will parse the same `NEEDS:` signals and ask via message instead of CLI prompt).
