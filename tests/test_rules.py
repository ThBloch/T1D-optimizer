...
class TestThomasRules(unittest.TestCase):
    ...
    def test_hypo_overrides_high_fasting(self):
        ...
        dose, reasoning = thomas_rules(20, 15.0, 1, 12.0)
        self.assertEqual(dose, 19)
        self.assertFalse(any('15.0' in r for r in reasoning))

    def test_activity_stacks_on_glucose(self):
        # High fasting (+2) plus mid-high strain s1=14 (-1u, tier 13-15) -> net +1u
    ...
    def test_activity_at_old_threshold_now_neutral(self):
        # s1=12.0 falls in neutral band (11 <= s1 < 13) -> no strain adjustment
        dose, _ = thomas_rules(20, 8.0, 0, 12.0)
        self.assertEqual(dose, 20)

    def test_clamp_upper_bound(self):
        # 28 + 3 = 31, clamped to DOSE_MAX
        dose, reasoning = thomas_rules(28, 15.0, 0, 12.0)
        self.assertEqual(dose, DOSE_MAX)
        self.assertTrue(any('Clamped' in r for r in reasoning))

    def test_clamp_lower_bound(self):
        # 16 - 2 (hypos) - 1 (s1=14 tier 13-15) = 13, clamped to DOSE_MIN
    ...
    def test_custom_threshold_param_lowers_trigger(self):
        ...
        dose, _ = thomas_rules(20, 9.0, 0, 12.0, fasting_lo=8.0)
        self.assertEqual(dose, 21)

    def test_new_pen_minus_one(self):
        dose, reasoning = thomas_rules(20, 8.0, 0, 12.0, new_pen=True)
        self.assertEqual(dose, 19)
    ...
    def test_new_pen_stacks_with_glucose_and_activity(self):
        # fasting 11.0 -> +1u (fallback); s1=13.0 in tier 13-15 -> -1u; new_pen -> -1u; net -1u
    ...
    def test_slope_none_falls_back_to_fasting(self):
        ...
        dose, reasoning = thomas_rules(20, 11.0, 0, 12.0)
        self.assertEqual(dose, 21)
    ...
    def test_strain_tier_plus3_below_6(self):
        dose, reasoning = thomas_rules(20, None, 0, 5.0)
        self.assertEqual(dose, 23)
        self.assertTrue(any('+3u' in r for r in reasoning))

    def test_strain_tier_plus2_boundary(self):
        # s1=6 is NOT < 6, falls to [6, 9) -> +2u
        dose, reasoning = thomas_rules(20, None, 0, 6.0)
        self.assertEqual(dose, 22)
        self.assertTrue(any('+2u' in r for r in reasoning))

    def test_strain_tier_plus1(self):
        dose, reasoning = thomas_rules(20, None, 0, 10.0)
        self.assertEqual(dose, 21)
        self.assertTrue(any('+1u' in r for r in reasoning))

    def test_strain_tier_neutral(self):
        # s1=12 in neutral band [11, 13) -> no strain adjustment
    ...
    def test_strain_tier_minus2_at_and_above_15(self):
        # s1=15 is NOT < 15, falls to else -> -2u
        dose, reasoning = thomas_rules(20, None, 0, 15.0)
        self.assertEqual(dose, 18)
        self.assertTrue(any('-2u' in r for r in reasoning))
